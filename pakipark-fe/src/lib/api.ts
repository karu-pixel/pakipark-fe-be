...
export const backendApi = {
  async login(
    ...
    const user = await apiRequest<LoginResult>('/auth/login', {
      method: 'POST',
      body: JSON.stringify({ identifier, password, twoFactorCode, app, keepLoggedIn }),
    });
  ...
  async setup2FA() {
    return apiRequest<{ secret: string; otpUri: string; message: string }>('/users/2fa/setup', {
      method: 'POST',
      auth: true,
    });
  },
  ...
  async verify2FA(code: string) {
    return apiRequest<{ message?: string }>('/users/2fa/verify', {
      method: 'POST',
      auth: true,
  ...
  async disable2FA(password: string) {
    return apiRequest<{ message?: string }>('/users/2fa/disable', {
      method: 'POST',
      auth: true,
