...
import { AdminFAQModal } from '@features/customer/components/CustomerModals';
import { colors } from '@theme/colors';
import { spacing } from '@theme/spacing';
import { backendApi, getSavedUser, type ApiUser } from '../../../lib/api';
import type { Booking as ApiBooking } from '@features/customer/types';

type TabId = 'dashboard' | 'bookings';
type TutorialTargetKey = 'logo' | 'dashboard' | 'bookings' | 'help';

// Mocks removed. Using backendApi for live data.

function Header({
  onGuide, onFAQ, onNotif, onProfile, onLogout, adminName, dropdownOpen, onToggleDropdown, logoRef, helpRef, unreadCount,
}: Readonly<{
  onGuide: () => void; onFAQ: () => void; onNotif: () => void; onProfile: () => void;
  onLogout: () => void;
  ...
  return (
    <View style={styles.header}>
      ...
      <View style={styles.headerActions}>
        ...
        <TouchableOpacity style={styles.iconBtn} accessibilityLabel="FAQs" onPress={onFAQ}>
        ...
        <TouchableOpacity style={styles.iconBtn} accessibilityLabel="Notifications" onPress={onNotif}>
          ...
          {(unreadCount ?? 0) > 0 && <View style={styles.notifDot} />}
        </TouchableOpacity>
...
function StatCards({ stats: liveStats }: Readonly<{ stats: any }>) {
  const stats = [
    {
      label: 'Total Bookings', value: String(liveStats?.totalBookings || 0), change: liveStats?.bookingTrend || '+0%',
      iconBg: '#DBEAFE', changeColor: colors.iconBlue,
      icon: <Ionicons name="car-outline" size={26} color={colors.iconBlue} />,
      decorBg: '#EFF6FF',
    },
    {
      label: 'Active Users', value: String(liveStats?.activeUsers || 0), change: liveStats?.userTrend || '+0%',
      iconBg: '#D1FAE5', changeColor: colors.iconGreen,
      icon: <Ionicons name="people-outline" size={26} color={colors.iconGreen} />,
      decorBg: '#ECFDF5',
    },
    {
      label: 'Parking Spots', value: String(liveStats?.totalSpots || 0), change: String(liveStats?.availableSpots || 0),
      iconBg: '#FEF3C7', changeColor: colors.iconYellow,
      icon: <Ionicons name="location-outline" size={26} color={colors.iconYellow} />,
      decorBg: '#FFFBEB',
    },
    {
      label: 'Revenue', value: `₱${(liveStats?.totalRevenue || 0).toLocaleString()}`, change: liveStats?.revenueTrend || '+0%',
...
function StatusBadge({ status }: Readonly<{ status: string }>) {
  ...
  let cfg: { bg: string; text: string; icon: 'checkmark-circle-outline' | 'time-outline'; label: string };
  
  if (s === 'active' || s === 'upcoming') {
    cfg = { bg: colors.statusActive, text: colors.statusActiveText, icon: 'checkmark-circle-outline', label: 'Active' };
  } else if (s === 'completed') {
    cfg = { bg: colors.statusCompleted, text: colors.statusCompletedText, icon: 'checkmark-circle-outline', label: 'Completed' };
  } else {
    cfg = { bg: colors.statusPending, text: colors.statusPendingText, icon: 'time-outline', label: 'Cancelled' };
  }
...
function BookingRow({ booking }: Readonly<{ booking: ApiBooking }>) {
  return (
    <View style={styles.bookingRow}>
      <View style={styles.bookingLeft}>
        <Text style={styles.bookingName}>{booking.location}</Text>
...
export function AdminHomeScreen({ onLogoutToAuth, onNavigateToPakiShip }: Readonly<{ onLogoutToAuth?: () => void; onNavigateToPakiShip?: () => void }>) {
  ...
  const tutorialTarget = useMemo<TutorialTargetKey>(() => {
    ...
    if (tutorialStep === 2) return 'bookings';
    return 'help';
  }, [tutorialStep]);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [s, b, n, users] = await Promise.all([
        backendApi.getAdminStats({}),
        backendApi.getAllBookings({ limit: 5 }),
        backendApi.getNotifications(1, 1),
        backendApi.getUsers().catch(() => []),
      ]);
      const profileCount = Array.isArray(users) ? users.length : 0;
      setStats({
        ...s,
        totalUsers: profileCount || s?.totalUsers || 0,
        activeUsers: profileCount || s?.activeUsers || 0,
      });
      setRecentBookings(b);
  ...
  const updateSpotlightLayout = useCallback((target: TutorialTargetKey, x: number, y: number, width: number, height: number) => {
    setSpotlightLayouts((current) => {
      const next = { ...current };
      next[target] = { x, y, width, height };
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ref.measureInWindow((x: number, y: number, width: number, height: number) => {
        updateSpotlightLayout(tutorialTarget, x, y, width, height);
      });
    }, 90);
    return () => clearTimeout(timer);
  }, [tutorialOpen, tutorialTarget, activeTab, updateSpotlightLayout]);

  const renderRecentBookingsContent = () => {
    if (loading) {
      return <ActivityIndicator size="small" color={colors.orange} style={{ marginVertical: 20 }} />;
    }
    if (recentBookings.length === 0) {
      return <Text style={{ textAlign: 'center', marginVertical: 20, color: colors.muted }}>No bookings yet.</Text>;
    }
    return recentBookings.map((b, i) => (
      <View key={b.id}>
        <BookingRow booking={b} />
        {i < recentBookings.length - 1 && <View style={styles.divider} />}
      </View>
    ));
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        {activeTab === 'dashboard' && (
          <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
            ...
            <Text style={styles.pageSubtitle}>
              Welcome back, {currentUser?.name || 'Admin'}! Here is what is happening with your parking facility today.
            </Text>
            ...
            <View style={styles.card}>
              ...
              {renderRecentBookingsContent()}
            </View>
            <View style={[styles.card, { paddingBottom: 16 }]}>
              <Text style={[styles.cardTitle, { marginBottom: 4 }]}>Quick Actions</Text>
              <Text style={{ fontSize: 12, color: '#9CA3AF', marginBottom: 16 }}>
                Fast shortcuts to handle reservations and parking slots.
              </Text>
              <TouchableOpacity style={[styles.actionRow, { backgroundColor: '#FFF7ED', marginBottom: 0 }]} accessibilityLabel="New Booking" onPress={() => setActiveTab('bookings')}>
      ...
      <View style={styles.tabBar}>
        {([
          ...
          return (
            ...
            >
              {/* Compact ref target — only wraps icon + label, not full flex width */}
              <View
                ref={tab.id === 'dashboard' || tab.id === 'bookings' ? captureTutorialRef(tab.id) : undefined}
                style={styles.tabSpotlightTarget}
              >
      ...
      <AdminFAQModal visible={showFAQModal} onClose={() => setShowFAQModal(false)} />
...
const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: '#1C436B' },
...
headerLeft: {
  ...
  tabItem: { flex: 1, alignItems: 'center', gap: 0 },
  // Tight inner wrapper used as the tutorial spotlight measurement target.
  // Not flex:1, so measureInWindow returns the content's natural width.
  tabSpotlightTarget: { alignItems: 'center', gap: 3, paddingHorizontal: 6 },
