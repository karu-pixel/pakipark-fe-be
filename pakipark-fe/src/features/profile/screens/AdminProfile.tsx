...
export function AdminProfile({ onBack }: AdminProfileProps) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      setUser(me);
      const [locs, statsData, users] = await Promise.all([
      ...
      setUsersCount(Array.isArray(users) && users.length > 0 ? users.length : statsData?.activeUsers || 0);
      await fetchRates();
    } catch (err) {
      ...
      >
        <View style={s.hero}>
          ...
          <View style={s.heroContent}>
            ...
            <Text style={s.heroName} numberOfLines={1}>{profile.firstName ? `${profile.firstName} ${profile.lastName}` : profile.name}</Text>
            <Text style={s.heroEmail} numberOfLines={1}>{profile.email || profile.phone || 'No contact added'}</Text>
...
const s = StyleSheet.create({
  ...
  nav: { minHeight: 58, flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', paddingHorizontal: 16, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#E5E7EB' },
  backBtn: { width: 38, height: 38, borderRadius: 19, backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E5E7EB', alignItems: 'center', justifyContent: 'center' },
  navTitle: { flex: 1, fontSize: 18, fontWeight: '900', color: '#1C436B', textAlign: 'center' },
  ...
  heroContent: { padding: 18, alignItems: 'center', zIndex: 1 },
  ...
  heroName: { maxWidth: '100%', fontSize: 19, fontWeight: '900', color: '#fff', marginBottom: 3, textAlign: 'center' },
  heroEmail: { maxWidth: '100%', fontSize: 13, color: 'rgba(255,255,255,0.72)', marginBottom: 14, textAlign: 'center' },
