    ...
    <View style={styles.headerCard}>
      <View style={styles.headerTop}>
        ...
        <View style={styles.headerActions}>
          ...
          <Pressable onPress={onFAQPress} style={styles.iconButton} accessibilityLabel="Open FAQs">
          ...
          >
            <View style={styles.avatar}>
              {profile_picture ? (
                <Image source={{ uri: profile_picture }} style={styles.avatarImage} />
              ) : (
        ...
        <Pressable style={styles.modalOverlay} onPress={() => setShowNotifications(false)}>
          <Pressable style={[styles.notificationPopup, notificationPosition]} onPress={(e) => e.stopPropagation()}>
            ...
            {notifications.length > 0 ? (
              <ScrollView style={{ maxHeight: 320 }} contentContainerStyle={styles.notificationPopupList}>
      ...
      <View style={styles.verticalList}>
        {cars.length === 0 ? (
          <View style={styles.emptyVehicleState}>
            <MaterialCommunityIcons name="car-off" size={42} color="#E5E7EB" />
            <Text style={styles.emptyVehicleTitle}>No Vehicles Yet</Text>
            <Text style={styles.emptyVehicleText}>Register your first vehicle to start booking parking spots.</Text>
          </View>
        ) : (
          cars.map((car, index) => (
            <Pressable
            ...
            >
              ...
              <Text style={styles.plate_number}>{car.plate_number}</Text>
      ...
      <View style={styles.recentBookingsList}>
        {items.length === 0 ? (
          <View style={styles.recentBookingsEmpty}>
            ...
            <Text style={styles.recentBookingsEmptyText}>Your recent parking history will appear here.</Text>
          </View>
        ) : (
          items.map((item, index) => (
            <View key={`${item.loc}-${item.date}-${index}`} style={styles.recentBookingCard}>
              <View style={styles.flexOne}>
...
const styles = StyleSheet.create({
  ...
  plate_number: { fontSize: 22, fontWeight: '800', color: COLORS.text, letterSpacing: 1 },
  ...
  emptyVehicleState: { alignItems: 'center', justifyContent: 'center', paddingVertical: 30, gap: 8 },
  emptyVehicleTitle: { fontSize: 16, fontWeight: '800', color: COLORS.text, marginTop: 4 },
  emptyVehicleText: { fontSize: 13, color: COLORS.muted, textAlign: 'center', paddingHorizontal: 20, lineHeight: 18 },
  recentBookingsEmpty: { alignItems: 'center', justifyContent: 'center', paddingVertical: 24, gap: 10, backgroundColor: '#FAFBFC', borderRadius: 16, borderWidth: 1, borderColor: '#F1F3F5', borderStyle: 'dashed' },
  recentBookingsEmptyText: { fontSize: 12, color: '#94A3B8', fontWeight: '600', textAlign: 'center', paddingHorizontal: 30 },
});
