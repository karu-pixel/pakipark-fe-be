...
export function BookingFlow({ location_id, location, address, cars, onBack, onBeforePayment, onConfirm }: BookingFlowProps) {
  ...
  const [hasParkingSlots, setHasParkingSlots] = useState(true);
  ...
  React.useEffect(() => {
    async function fetchSlots() {
      ...
      try {
        ...
        const hasPhysicalSlots = res.some((item: any) => Number(item.totalSpots || 0) > 0);
        setHasParkingSlots(hasPhysicalSlots);
  ...
  const handleSlotPress = (slot: TimeSlot) => {
    if (!hasParkingSlots) {
      Alert.alert('No Available Parking Slot', 'This location has no available parking slot.');
      return;
    }
  ...
  const handleConfirmPay = async () => {
    ...
    try {
      ...
      console.warn('PayMongo checkout error:', err);
  ...
  const renderDateTimeStep = () => (
    <View style={styles.dateTimeScreen}>
          ...
          <View style={styles.slotGrid}>
            {loadingSlots ? (
              ...
              <View style={styles.noSlotsCard}>
                ...
                <Text style={styles.noSlotsTitle}>No available parking slot</Text>
                <Text style={styles.noSlotsText}>This location does not have parking slots available for booking yet.</Text>
              </View>
              ...
              <View style={styles.noSlotsCard}>
                ...
                <Text style={styles.noSlotsTitle}>No time slots available</Text>
                <Text style={styles.noSlotsText}>Please choose another date or parking location.</Text>
              </View>
            ) : (
      ...
      <View style={styles.dateTimeActions}>
        ...
        >
          <Text style={styles.prototypeCtaText}>{canContinue ? `Next: Payment - ₱${totalAmount}` : hasParkingSlots ? 'Select a Time Range to Continue' : 'No Available Parking Slot'}</Text>
        </Pressable>
      </View>
...
const styles = StyleSheet.create({
  ...
  noSlotsTitle: { color: COLORS.navy, fontSize: 15, fontWeight: '900', textAlign: 'center' },
  noSlotsText: { color: COLORS.subtle, fontSize: 11, fontWeight: '700', lineHeight: 16, textAlign: 'center', marginTop: 5 },
  slotButton: {
    width: '31%',
    minWidth: 93,
    height: 57,
