...
interface LoginFormMainProps {
  formData: { identifier: string; password: string; twoFactorCode: string };
  setFormData: Dispatch<SetStateAction<{ identifier: string; password: string; twoFactorCode: string }>>;
  errors: { identifier: string; password: string; twoFactorCode: string };
...
export function LoginFormMain({
  ...
  const [focusedField, setFocusedField] = useState<'identifier' | 'password' | 'twoFactorCode' | null>(null);
  ...
  return (
    <View style={styles.root} testID="login-form-main">
      ...
      {two_factor_required && (
        <View style={styles.alertBox}>
          <Text style={styles.alertIcon}>2</Text>
          <Text style={styles.alertText}>Enter your authenticator code.</Text>
        </View>
      )}
      ...
      {two_factor_required && (
        <View style={styles.fieldGroup}>
          ...
          <View style={[styles.inputShell, focusedField === 'twoFactorCode' ? styles.inputShellFocused : undefined]}>
            <Feather name="key" size={14} color={focusedField === 'twoFactorCode' ? loginColors.accent : '#B2C0CF'} />
            <TextInput
              value={formData.twoFactorCode}
              onChangeText={(twoFactorCode) => setFormData((prev) => ({ ...prev, twoFactorCode: twoFactorCode.replace(/\D/g, '').slice(0, 6) }))}
              onFocus={() => setFocusedField('twoFactorCode')}
              onBlur={() => setFocusedField(null)}
              placeholder="123456"
              placeholderTextColor={loginColors.subtle}
              keyboardType="number-pad"
              autoCapitalize="none"
              style={[styles.input, errors.twoFactorCode ? styles.inputError : undefined]}
            />
          </View>
          {!!errors.twoFactorCode && <Text style={styles.errorText}>{errors.twoFactorCode}</Text>}
        </View>
      )}
      ...
      <Pressable
        style={[styles.primaryButton, loginShadows.button, isSubmitting ? styles.primaryButtonDisabled : undefined]}
        onPress={onSubmit}
        disabled={isSubmitting}
      >
        <Text style={styles.primaryButtonText}>{isSubmitting ? 'Signing In...' : two_factor_required ? 'Verify and Continue' : 'Continue to Dashboard'}</Text>
      </Pressable>
