...
type BookingStatus = 'all' | 'upcoming' | 'active' | 'completed' | 'cancelled' | 'no_show';

const FILTERS: BookingStatus[] = ['all', 'upcoming', 'active', 'completed', 'cancelled', 'no_show'];
      ...
      <View style={styles.confirmedOverlay}>
        <SafeAreaView style={styles.confirmedSheet}>
          ...
          <View style={styles.confirmedActions}>
            {(currentBooking.status as string) === 'upcoming' && (
              <>
                <Pressable
                  style={[styles.confirmedBtnPrimary, { flex: 1.2 }, isActioning && { opacity: 0.7 }]}
                  onPress={async () => {
                    await onCheckIn(currentBooking.id);
                    onClose();
                  }}
                  disabled={isActioning}
                >
                  {isActioning ? <ActivityIndicator size="small" color="#fff" /> : <Text style={styles.confirmedBtnPrimaryText}>Check In</Text>}
                </Pressable>
                <Pressable
                  style={[styles.confirmedBtnSecondary, { flex: 1 }, isActioning && { opacity: 0.7 }]}
                  onPress={() => {
                    onNoShow(currentBooking.id);
                    onClose();
                  }}
                  disabled={isActioning}
                >
                  <Text style={styles.confirmedBtnSecondaryText}>No Show</Text>
                </Pressable>
              </>
            )}
                  ...
                  <View style={styles.bookingBottom}>
                    ...
                    <View style={styles.bottomButtons}>
                      ...
                      {(booking.status as string) === 'upcoming' && (
                        <>
                          <Pressable
                            style={[styles.actionInline, { backgroundColor: '#DDFBEA', borderColor: '#14964B' }]}
                            onPress={() => handleCheckIn(booking.id)}
                            disabled={isActioning}
                          >
                            {isActioning ? <ActivityIndicator size="small" color="#14964B" /> : <Text style={[styles.actionInlineText, { color: '#14964B' }]}>Check In</Text>}
                          </Pressable>
                          <Pressable
                            style={[styles.actionInline, { backgroundColor: '#F3F4F6', borderColor: '#9CA3AF' }]}
                            onPress={() => handleNoShow(booking.id)}
                            disabled={isActioning}
                          >
                            <Text style={[styles.actionInlineText, { color: '#6B7280' }]}>No Show</Text>
                          </Pressable>
                        </>
                      )}
