...
export class LegacyHandlerService {
  ...
  private async getLoginEmailCandidates(identifier: string) {
    ...
    const rows = matchingRows.flatMap((match) => Array.isArray(match) ? match : match ? [match] : []);
  ...
  private async findLoginProfile(raw: string, normalizedPhone: string) {
    ...
    const { data, error } = isEmail(raw)
      ? await query.eq('email', raw.toLowerCase())
      : normalizedPhone
        ? await query.eq('phone', normalizedPhone)
  ...
  private async getParkingSlotSnapshot(
    ...
    if (!slots.length) {
      throw new BadRequestException('No available parking slot for this location.');
    }
  ...
  private async booking(action: string, req: RequestWithUser) {
    ...
    if (action === 'updateBookingStatus') {
      ...
      const adminTable = this.client.schema('reservation').from('bookings');

      const { data: booking, error: bookingError } = await adminTable
        .select('*')
        .eq('id', req.params.id)
        .maybeSingle();
      ...
      if (nextStatus === 'cancelled' || nextStatus === 'completed') {
        await Promise.all([
          this.adjustLocationAvailableSpots(this.client, booking.location_id, 1),
          this.updateParkingSlotStatus(this.client, booking.parking_slot_id, 'available'),
        ]);
      } else if (nextStatus === 'active') {
        await this.updateParkingSlotStatus(this.client, booking.parking_slot_id, 'occupied');
      }
    ...
    if (action === 'checkOutBooking') {
      ...
      if (error) throw new BadRequestException(error.message);
      if (!data) throw new BadRequestException('Failed to check out booking. It may have been modified by another process.');
  ...
  private async location(action: string, req: RequestWithUser) {
    ...
    if (action === 'getLocations') {
      ...
      const availableSlotCounts = new Map<string, number>();
      for (const slot of slots || []) {
        const key = String(slot.location_id);
        availableSlotCounts.set(key, (availableSlotCounts.get(key) || 0) + 1);
      }

      const normalized = ownedLocations.map((row: any) => {
        const availableSlots = availableSlotCounts.get(String(row.id)) || 0;
        return {
          ...this.normalizeLocation(row),
          availableSpots: availableSlots,
          available_spots: availableSlots,
          distance: availableSlots > 0 ? `${availableSlots} spots available` : 'No available parking slot',
          availabilityLabel: availableSlots > 0 ? `${availableSlots} spots available` : 'No available parking slot',
          hasAvailableParkingSlot: availableSlots > 0,
        };
      });

      return { success: true, data: normalized, total: normalized.length };
    }
  ...
  private async analytics(action: string, req: RequestWithUser) {
    ...
    if (action === 'getDashboardStats') {
      ...
      const { count: profileCount, error: profileCountError } = await this.client
        .schema('account')
        .from('profiles')
        .select('id', { count: 'exact', head: true });
      if (profileCountError) throw new BadRequestException(profileCountError.message);
      ...
      const activeRows = rows.filter((row: any) => ['upcoming', 'active', 'payment_pending'].includes(String(row.status)));
      const activeBookings = activeRows.length;
      return {
        success: true,
        data: {
  ...
  private async availableSlots(locationId: string, date?: string) {
    ...
    const totalSpots = activeSlots.length;
